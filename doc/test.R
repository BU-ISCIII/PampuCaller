...
draw_graph <- function(reg,dir,set){
		...
		pdf(paste(dir,"/",reg[4],"_zoom_0.015",sep=""),width=40)
			print(g[[1]])
			print(g[[2]])
		dev.off()

		g <- graph_barerror(set,start_end=reg,zoom=c(0,0.008))
		pdf(paste(dir,"/",reg[4],"_zoom_0.008",sep=""),width=40)
			print(g[[1]])
			print(g[[2]])
		dev.off()

		g <- graph_barerror(set,start_end=reg,zoom=c(0,0.5))
		pdf(paste(dir,"/",reg[4],"_zoom_0.5",sep=""),width=40)
			print(g[[1]])
			print(g[[2]])
		dev.off()

		g <- graph_barerror(set,start_end=reg,zoom=c(0,0.25))
		pdf(paste(dir,"/",reg[4],"zoom_0.25",sep=""),width=40)
			print(g[[1]])
			print(g[[2]])
		dev.off()
